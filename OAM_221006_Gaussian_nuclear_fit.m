...
function [p_nuc]=OAM_221006_Gaussian_nuclear_fit(IG,peak_cutoff,x_size,y_size,Lcells,ccell)

%     IG=IGreen1;
    p_nuc=zeros(size(IG)); % allocate
    %-----gaussian fit--------------------
    h = fspecial('average', 5); % structuring element for filtering (kernel)
    I_gf=  imfilter(IG,h,'replicate');% filtering for bringing up spatial features
    Ignew=I_gf;
